...
const CHANNELS = ['1205838668598870037','1205838668598870040']
...
client.on('messageCreate', async (message) => {
    ...
    prevMessages.forEach((msg) => {
        ...
        //this will run if the convo belongs to a regular user
        conversation.push({
            role: 'user',
            name: 'username',
            content: msg.content,
...
//responde to messge by sending a request to openAI
const response = await openai.chat.completions.create({
    model: 'gpt-3.5-turbo',
    messages: conversation,

}).catch((erros) => console.error('OpenAI Error:\n', error));
...
//breakdown message chara because of discord limit

const respondeMessage = response.choices[0].message.content;
const chunkSizeLimit = 2000;

for (let i =0; i < respondeMessage.length; i+= chuckSizeLimit) {
